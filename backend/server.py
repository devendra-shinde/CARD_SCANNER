...
async def signup(body: SignupIn):
    ...
    if existing:
        ...
        await _issue_otp(body.email.lower(), "signup")
        return {"message": "OTP re-sent", "email": body.email.lower()}
    ...
    await _issue_otp(body.email.lower(), "signup")
    return {"message": "OTP sent to email", "email": body.email.lower()}
...
async def login(body: LoginIn):
    ...
    if not user.get("email_verified"):
        await _issue_otp(user["email"], "signup")
        raise HTTPException(403, "Email not verified. OTP re-sent.")
    token = make_access_token(user["id"])
    return {
        "access_token": token,
        "token_type": "bearer",
...
async def forgot_password(body: ForgotPwIn):
    ...
    await _issue_otp(user["email"], "reset")
